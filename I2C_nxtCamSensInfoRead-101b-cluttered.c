...
task main()
{
	...
	while (true)
	{
		...
		//char c1=0x56;
		//string dest;
    //stringFromChars(sToString, sFromChars);
		//stringFromChars(dest, &nReplyBytes[0]);
    // --- > displayTextLine(0, "%s", &nReplyBytes[0]);
    displayTextLine(0, "%s", &nReplyBytes);
		//displayTextLine(2, "%s", dest);
		//displayString(0, "%d", nReplyBytes[0]);
		//displayString(nLineNumber, sFormatString, ...);
		//displayTextLine(0, "%x", nReplyBytes[0]);
		//displayTextLine(0, "%d", sizeof(nReplyBytes));
		//writeDebugStreamLine("I2C reply results (sizeof(nReplyBytes),nReplyBytes[0]), %d, %s", sizeof(nReplyBytes),nReplyBytes[0]);
		//writeDebugStreamLine("I2C reply results (sizeof(nReplyBytes)), %d", sizeof(nReplyBytes));
