...
task main()
{
	...
	const int kRegisterIndex0 = 0x00;    // You may want to make this a function parameter
	const int kRegisterIndex8 = 0x08;    // You may want to make this a function parameter
	const int kRegisterIndex10 = 0x10;   // You may want to make this a function parameter
	...
	/*
	static const ubyte nReadDeviceVerNum[] =
	{
	2,                   // This is length field for transmitted message.
	kI2CAddress,         // The I2C address of the device. Almost all devices use value '0x02'
	kRegisterIndex0,     // The internal register index within the sensor to start writing at.
	};


	static const ubyte nReadVendorID[] =
	{
	2,                   // This is length field for transmitted message.
	kI2CAddress,         // The I2C address of the device. Almost all devices use value '0x02'
	kRegisterIndex8,     // The internal register index within the sensor to start writing at.
	};
	...
	2,                   // This is length field for transmitted message.
	kI2CAddress,         // The I2C address of the device. Almost all devices use value '0x02'
	kRegisterIndex10,     // The internal register index within the sensor to start writing at.
	};

	*/

	//  =============== Try ==> memset to update value of const 'kRegisterIndex' ====================  //
	...
	while (true)
	{
		...
		//read device sw version
		memset(pnReadDevice + 2, 0x00, 1);
		...
		displayTextLine(0, "%s", &nReplyBytes[0]);


		//read vendor ID
		memset(pnReadDevice + 2, 0x08, 1);
		sendSensorRead(S1, nReadDevice[0], kReplyLength);
		while (!readSensorReply(S1, &nReplyBytes[0], kReplyLength))
		{
			//wait for read to return bool 'true'
		...
		displayTextLine(1, "%s", &nReplyBytes[0]);

		//read Device ID
		memset(pnReadDevice + 2, 0x10, 1);
		sendSensorRead(S1, nReadDevice[0], kReplyLength);
		while (!readSensorReply(S1, &nReplyBytes[0], kReplyLength))
		{
			//wait for read to return bool 'true'
		...
		displayTextLine(2, "%s", &nReplyBytes[0]);


		/*

		//read device sw version
		sendSensorRead(S1, nReadDeviceVerNum[0], kReplyLength);
		while (!readSensorReply(S1, &nReplyBytes[0], kReplyLength))
		{
		//wait for read to return bool 'true'
		sleep(5);
		...
		//read vendor ID
		sendSensorRead(S1, nReadVendorID[0], kReplyLength);
		while (!readSensorReply(S1, &nReplyBytes[0], kReplyLength))
		{
		//wait for read to return bool 'true'
		sleep(5);
		...
		*/
		//displayTextLine(1, "%d",kReplyLength);  //for testing
