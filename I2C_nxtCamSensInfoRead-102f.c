...
task main()
{
	...
	while (true)
	{
		...
		for (int i=0; i<3; i++)
		{
			if (i==0) memset(pnReadDevice + 2, 0x00, 1);  //read device sw version
				if (i==1) memset(pnReadDevice + 2, 0x08, 1);  //read vendor ID
				if (i==2) memset(pnReadDevice + 2, 0x10, 1);  //read Device ID
			...
			displayTextLine(i, "%s", &nReplyBytes[0]);
