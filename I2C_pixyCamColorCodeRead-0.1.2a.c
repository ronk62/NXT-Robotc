...
task main()
{
	...
	const short ColorCodeByte1 = 0x73;    // You may want to make this a function parameter
	//const char ColorCodeByte2 = 0x03;    // You may want to make this a function parameter

	const int kReplyLength = 6;          // You might want to make this a function parameter.
	ubyte nReplyBytes[kReplyLength];

	static const short nReadDevice1[] =
	{
		3,                   // This is length field for transmitted message.
		kI2CAddress,         // The I2C address of the device. Almost all devices use value '0x02'
	...
	const ubyte *pnReadDevice1=&nReadDevice1;
	...
	while (true)
	{
		...
		while (!I2Cerr)
		{
			...
			//Replaced call to 'sendSensorRead' with the below; this was added for debugging color code reads
			...
			//{
				...
				//displayTextLine(i, "%d", nReadDevice1[i]);

				//sleep(2000);
			//}

			sendI2CMsg(S1, pnReadDevice1, kReplyLength);
