...
task main()
{
	...
	while (true)
	{
		...
		if (colorDetected==0x01 && xPos>140) moveMotorA(25,90); //turn left
		if (colorDetected>0x01 && xPos<190) moveMotorA(25,90); //turn left
		if (colorDetected==0x01 && xPos<90) moveMotorA(-25,-90); //turn right
		if (colorDetected==0x00) noBlobCount++; //no blob detected - increment noBlobCount
		if (noBlobCount>14)
    {
      moveMotorA(25,90); //turn left
