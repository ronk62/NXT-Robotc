...
void moveMotorA(short pwr, long target)
{
	...
	nMotorEncoder[motorA]=0; //reset encoder
	nMotorEncoderTarget[motorA]=target-20;
	motor[motorA] = -1*pwr;
...
task main()
{
	...
	while (true)
	{
		...
		if (colorDetected==0x01 && xPos>120) moveMotorA(-55,110); //turn right
		if (colorDetected>0x01 && xPos<190) moveMotorA(55,110); //turn left
		if (colorDetected==0x01 && xPos<90) moveMotorA(-55,110); //turn right
		if (colorDetected==0x00) noBlobCount++; //no blob detected - increment noBlobCount
		if (noBlobCount>14)
    {
      moveMotorA(55,110); //turn left
