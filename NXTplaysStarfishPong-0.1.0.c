...
void moveMotorA(short pwr, long target)
{
	...
	wait1Msec(70); //wait before continuing

	nMotorEncoder[motorA]=0; //reset encoder
	nMotorEncoderTarget[motorA]=target-5;
	motor[motorA] = -1*pwr;
...
{
	const int kI2CAddress    = 0x02;     // You may want to make this a function parameter
	const int kRegisterIndex1 = 0x51;    // You may want to make this a function parameter
	const int kRegisterIndex2 = 0x52;    // You may want to make this a function parameter

	const int kReplyLength = 5;          // You might want to make this a function parameter.
	...
	static const ubyte nReadDevice2[] =
	{
		2,                   // This is length field for transmitted message.
		kI2CAddress,         // The I2C address of the device. Almost all devices use value '0x02'
		kRegisterIndex2,     // The internal register index within the sensor to start writing at.
	};
  ...
  // ubyte *pnReadDevice=&nReadDevice;
	...
	while (true)
	{
		...
		//set signature 1 vars to nReplyBytes values
		yPosSig1=nReplyBytes[2];


		sendSensorRead(S1, nReadDevice2[0], kReplyLength);  // read signature 2
		while (!readSensorReply(S1, &nReplyBytes[0], kReplyLength))
		{
			//wait for read to return bool 'true'
			sleep(50);
		...
		*/
		...
		//set signature 2 vars to nReplyBytes values
		yPosSig2=nReplyBytes[2];

		//evaluate detected blob color and location; actuate motor as needed
		if (yPosSig1>yPosSig2+5) moveMotorA(-25,50); //move Red Shark up
    if (yPosSig1<yPosSig2-5) moveMotorA(25,50); //move Red Shark down
