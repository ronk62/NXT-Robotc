...
/**
* This program will use the pixycam to track a color-coded object using PIXYCreadGeneralQry
* The pixycam is mounted to a differential drive platform, which will move according to
* the pixycam input
*
* Revisions:
...
task main()
{
	...
	int motorBpwr=0;
	int topSpeedLim=8;  // set this to a very low value for initial testing
	int pwrScaling=7;   // eg. motorBpwr = motorBpwr/pwrScaling
  ...
  */
  ...
  while (true) {
		...
		_numberOfBlocks = _sigVal; // sogVal of 0 means no blocks found
		if (_numberOfBlocks == 0)
		{
			if (time1[T1] > 3000)
			{
		    nSyncedTurnRatio = -100;
		    offset_xPos= 190; // ~75% of 255
			}
		}
