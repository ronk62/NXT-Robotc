	...
	*/
	...
	while (true) {
		...
		for (_qryAddress = 0x51; _qryAddress < 0x53; _qryAddress ++)
		{
			...
			fwdRevPwr = (100 - yAvgS1)/7 ; // fwd or Rev motion at speed based on object area
			...
			{
				clearTimer(T1); // clear (reset) timer 1, which is used to detect how long since last object detected
				offset_xPos = xAvgS1 - 127; // 127 is ~255/2
			}
			...
			displayTextLine(1, "xAvgS1: %d", xAvgS1);
			...
			displayTextLine(6, "yAvgS1: %d", yAvgS1);
