...
#pragma config(Motor,  motorA,          panMotor,      tmotorNXT, PIDControl, encoder)
#pragma config(Motor,  motorB,          tiltMotor,     tmotorNXT, PIDControl, encoder)
  ...
  ubyte _angle=0;
	...
	while (true)
	{
	  ...
	  while (!PIXYCreadColorCodeQry(PIXYC, _ColorCodeByte1, _ColorCodeByte2, _numberOfBlocks, _xPos, _yPos, _width, _height, _angle))
			...
			if (_numberOfBlocks==1 && _xPos<150) motor[panMotor]= 11;
			sleep(100);
			motor[panMotor]=0;

			if (_numberOfBlocks==1 && _xPos>160) motor[panMotor]= -11;
			sleep(100);
			motor[panMotor]=0;
